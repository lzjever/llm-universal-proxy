...
当前容器版本是 `v0.2.23`；生产环境请 pin
`ghcr.io/agentsmith-project/llm-universal-proxy:v0.2.23` 或已发布 digest，
不要依赖 `latest`。容器运行、Docker Compose、一分钟 smoke 验证、Admin
Dashboard 鉴权边界，以及认证拉取或 public 包匿名拉取的 GHCR 访问说明都放在
