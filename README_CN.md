...
| `LLM_UNIVERSAL_PROXY_AUTH_MODE` | 必填的 proxy auth mode；proxy 持有 provider key 时使用 `proxy_key` |
| `LLM_UNIVERSAL_PROXY_KEY` | `proxy_key` 模式必填；客户端 SDK API key 必须填这个值 |
...
- `LLM_UNIVERSAL_PROXY_AUTH_MODE` 是所有 data-plane 路由共用的进程级设置，不是 per-upstream YAML 或 API 字段；静态示例见 [docs/configuration.md](./docs/configuration.md)，runtime payload 映射见 [docs/admin-dynamic-config.md](./docs/admin-dynamic-config.md)
...
当前已发布容器版本是 `v0.2.24`；Cargo package version `0.2.25` 是下一次
release identity，并不是已发布容器 tag。生产环境请 pin
`ghcr.io/agentsmith-project/llm-universal-proxy:v0.2.24` 或已发布 digest，不要依赖
...
默认推荐静态 YAML。只有在你需要运行中改配置时，再用 admin 接口读取运行时状态或替换 namespace 配置。Admin payload 使用 runtime shape，data-plane auth mode 仍然来自进程环境。
