...
`RequestTranslationPolicy::default()` is `max_compat`, and `translate_request()` defaults to `max_compat` when no explicit policy is supplied. Same-format request translation passthrough preserves native request fields only on same-provider/native passthrough lanes. A compatible same-protocol lane keeps portable fields, but it must not be documented as native provider passthrough.

The proxy should treat function calling and explicit transcript replay as the common denominator. Hosted tools, provider-managed state, compaction, and cache-control semantics are increasingly vendor-specific and should be preserved only on same-provider/native passthrough lanes or documented as intentional degradations. Same-provider/native passthrough preserves opaque provider state when the route remains native.
