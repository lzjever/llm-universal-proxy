...
| Reasoning opaque state | `reasoning.encrypted_content`, reasoning item `encrypted_content` | No stable equivalent | No stable equivalent | No stable equivalent | Drop | Same-provider passthrough only; cross-provider request inputs fail closed. Never synthesize |
...
| Compaction | `context_management`, `/responses/compact`, compaction items | No stable equivalent | beta `context_management` compaction | No stable equivalent | Drop | Provider-native state transform |
