...
| Typed media input parts | `input_image`, `input_audio`, `input_file` | image, audio, and file content parts | image and document blocks | `inlineData` and `fileData` parts | Approx / Fail-closed | Translate only supported media in the effective surface and only source transports the target can represent. Gemini `inlineData` to OpenAI Chat/Responses remains preservable when the effective surface allows it, but all Gemini `fileData.fileUri` sources to OpenAI Chat/Responses fail closed until an explicit fetch/upload adapter exists. HTTP(S) URLs are distinct from provider-native or local URIs such as `gs://`, `s3://`, and `file://`; unsupported media or source forms, provider `file_id`, unknown typed parts, Anthropic remote image URL to Gemini, Gemini video to non-Gemini targets, and conflicting MIME provenance fail closed. |
...
| Hosted / server tool definitions | Rich Responses tool families | No official hosted/server tool family on Chat create | Server tools and MCP connector | Official built-in/server-side `Tool` branches plus `toolConfig` and `mcpServers` | Native-only / Fail-closed | Keep same-provider only |
...
| Reasoning request policy | `reasoning` | Model-specific | `thinking` | `thinkingConfig` | Fail-closed | Same idea, different execution contract |
...
| Reasoning opaque state | `reasoning.encrypted_content`, reasoning item `encrypted_content` | No stable equivalent | No stable equivalent | No stable equivalent | Native-only / Warn/drop opaque carrier / Fail-closed | Same-provider/native passthrough preserves the carrier. In default/max_compat request translation, warn/drop opaque carrier fields only when visible summary or visible transcript/history remains; strict/balanced fail closed, and opaque-only reasoning state fails closed. Never synthesize. Response-side reasoning encrypted_content has a separate Anthropic carrier recovery path. |
| Prompt-cache control | `prompt_cache_key`, retention policy | `prompt_cache_key` on supported surfaces | `cache_control` | `cachedContent` or cache API | Native-only / Fail-closed | Not the same primitive |
...
| Storage / persistence | `store` plus stored response resources | Official `store` field for stored completion artifacts | No official request-side persistence flag | Official `store` request flag | Native-only / Fail-closed | Storage semantics differ |
| Service tier | `service_tier` | Official `service_tier` request field | Official `service_tier` request field on current Messages surfaces | `serviceTier` | Native-only | Passthrough only; tier semantics are vendor-specific |
