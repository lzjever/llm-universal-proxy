...
| Reasoning opaque state | `reasoning.encrypted_content`, reasoning item `encrypted_content` | No stable equivalent | No stable equivalent | No stable equivalent | Drop | Same-provider/native passthrough only. In default/max_compat request translation, drop opaque carriers only when visible summary or visible transcript context remains; strict and balanced fail closed, and opaque-only state fails closed. Never synthesize |
...
| Compaction | `context_management`, `/responses/compact`, compaction items | No stable equivalent | beta `context_management` compaction | No stable equivalent | Drop | Native state surfaces stay same-provider only. In default/max_compat request translation, request-side compaction input items may drop opaque carrier fields when visible transcript or explicit summary text remains; opaque-only compaction still fails closed |
