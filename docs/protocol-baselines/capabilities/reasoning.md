...
| Encrypted / opaque reasoning state | Official docs now enumerate `reasoning.encrypted_content` in `include`; reasoning items can carry `encrypted_content` | No stable equivalent | No stable equivalent in the public Messages wire shape | No stable equivalent | Preserve only on native Responses passthrough. Cross-provider request translation fails closed, including proxy-local carriers for Anthropic signed or omitted thinking. Do not synthesize fake encrypted state. |
...
| Opaque reasoning state is non-portable | OpenAI's encrypted reasoning content has no safe Chat, Anthropic, or Gemini target shape. |
...
3. Fail closed for encrypted reasoning state, detailed block structure, and provider-specific effort controls across protocol boundaries.
