...
| Anthropic MCP and beta state surfaces | Current Anthropic MCP docs document a current beta header for the MCP connector, and current reference snippets expose beta fields like `context_management`, `container`, `service_tier`, and `mcp_servers` | These are implementation-risk areas and should be flagged as raw/native or beta-only paths |
...
| OpenAI background mode | Medium | It changes lifecycle expectations and can outlive a simple request/response mental model. | Treat as raw/native behavior only. |
