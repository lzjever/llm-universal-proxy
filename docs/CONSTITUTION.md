...
LLM Universal Proxy is a **format-agnostic protocol translation middleware** for Large Language Model APIs. Its singular mission is to serve as a maximally compatible bridge layer between LLM clients and LLM backends, regardless of which API protocol each side speaks.

## Mission Statement

**Enable any client to talk to any LLM backend through one stable proxy, with zero protocol lock-in.**

A client using OpenAI Chat Completions, OpenAI Responses, Anthropic Messages, or Google Gemini should be able to transparently reach any upstream LLM endpoint — whether it speaks the same protocol or a completely different one — without the client or the user needing to know or care about the underlying protocol mismatch.
...
|----------------|-------------------|------------|
| OpenAI Chat Completions | Any supported | Translate if needed |
| OpenAI Responses API | Any supported | Translate if needed |
| Anthropic Messages | Any supported | Translate if needed |
| Google Gemini | Any supported | Translate if needed |
...
- Any endpoint that implements one of the four supported protocols
